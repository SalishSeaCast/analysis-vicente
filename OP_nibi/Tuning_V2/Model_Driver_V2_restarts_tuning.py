...
def PBDEs_OP_run(year , month, day, sim_length, number_outputs, string,
    ...
    if restart and restart_filename is not None:
        # 1. Load existing particles from the restart file
        pset_states = ParticleSet.from_particlefile(fieldset=field_set, 
                                                    pclass=MPParticle, 
                                                    filename=restart_filename)
        
        # 2. Identify how many particles were supposed to exist in total 
        # based on the original release schedule
        total_release_seconds = day_release * 86400
        total_potential_particles = int(total_release_seconds / release_freq)
        
        # 3. Create the full timeline of release times
        all_release_times = np.arange(seconds_initial, 
                                      seconds_initial + (release_freq * total_potential_particles), 
                                      release_freq)
        
        # 4. Filter for particles that haven't been released yet
        # (Current time in simulation seconds relative to start_simulation)
        current_sim_seconds = (start_time - start_simulation).total_seconds()
        future_release_indices = all_release_times > current_sim_seconds
        future_times = all_release_times[future_release_indices]
        num_new_particles = len(future_times)

        if num_new_particles > 0:
            print(f"Adding {num_new_particles} new particles to the restart schedule.")
            
            # Create a temporary pset for the new particles
            pset_new = ParticleSet(field_set, pclass=MPParticle,
                                   lon=constants['Iona_clon'] * np.ones(num_new_particles),
                                   depth=constants['Iona_z'] * np.ones(num_new_particles),
                                   lat=constants['Iona_clat'] * np.ones(num_new_particles),
                                   time=future_times) # Parcels handles delayed release via 'time'
            
            # Merge new particles into the restarted set
            pset_states.add(pset_new)

        restart_output_dir = param['restart_output_dir']
        restart_basename = f"{name_extension}_restarted.zarr"
