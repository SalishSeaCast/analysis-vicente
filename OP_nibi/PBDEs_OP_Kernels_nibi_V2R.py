...
def PBDEs_states(particle, fieldset, time):    
    ...
    # 1. Release logic: Only check release_time if the particle is unreleased
    if particle.status < 0:
        if time > particle.release_time:
            particle.status = -particle.status
            
    # 2. State update logic: Only run if the particle is active
    if particle.status > 0:
        ...
        if particle.status == 1 and random_value < fieldset.deso_s_probability:
            ...
            particle.status = 3  # Adsorption to Marine Particle
        elif particle.status == 12:
            if random_value < fieldset.abso_sed_probability:
                particle.status = 13  # Adsorption to Marine Particle in Sediments
    ...
    if particle.status > 11:
        ...
        if e3t_val_o2 < fieldset.lowere3t_o2:
            ...
            if vtau_constant * (log_e3t - fieldset.log_z_star) ** 2 <= H_vel_2:
                ...
                particle.stuck += 1 
#                
######## NEW PART FOR SOLVING STUCK PARTICLES #########
    ...
    if particle.status > 10:
        if (uus == 0) and (vvs == 0):
            if (particle.fmask == 0) or (particle.stuck > 86500/5*30): # If not move after this, then displace towards the deepest box       
