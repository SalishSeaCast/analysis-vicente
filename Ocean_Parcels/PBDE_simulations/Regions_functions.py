...
def plot_vertical_total_state_profiles(array_total_profiles, array_status_profiles,
    ...
    for i in range(9):
        ...
        # Right: map
        ax_map.pcolormesh(nav_lon, nav_lat, tmask, cmap=cmap, shading='auto')
        ax_map.scatter(*site_coords[i], s=15, alpha=0.01, color='k', label=site_names[i])
        ax_map.scatter(water_regions_lon[i], water_regions_lat[i], s=0.05, color='blue', alpha=0.1, label='Water')
        ax_map.scatter(sedimented_regions_lon[i], sedimented_regions_lat[i], s=0.5, color='red', alpha=0.1, label='Sedimented')
        ax_map.set_xlim(water_regions_lon[i].mean() - 0.7, water_regions_lon[i].mean() + 0.7)
        ax_map.set_ylim(water_regions_lat[i].mean() - 0.7, water_regions_lat[i].mean() + 0.7)
