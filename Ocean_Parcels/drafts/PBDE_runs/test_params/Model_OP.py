...
def simulation_run(start_time, sim_length, number_particles):
    ...
    name_states = 'PBDEs_run_for_'+str(sim_length)+'_days_'+str(number_particles)+'_particles_' 
    ...
    def PBDEs_states(particle, fieldset, time):
        if particle.time <= 3600:#particle.time <= 3600particle.initialized == 0:
            n = particle.n 
    ...
    def PBDEs_forms(particle, fieldset, time):
        ...
        if particle.status == 1:
            sinkvel = 40*(dt_h) # m/hr * dt --> to seconds
...
#            particle_ddepth = 0     
    ...
    def resuspension(particle, fieldset, time):
        if particle.status == 4:
            threshold = 2 # threshold for particles to know when to resuspend
