...
def PBDEs_OP_run(start_time, sim_length, number_particles, days_release, delta_t, R_interval):
    ...
    deso_m = 1.6 / 24 # Marine Particle → Colloidal/Dissolved
    #
    dt_h = 1/3600
    # Status updates
    abso_prob = 1 - np.exp(-abso * dt_h)
    deso_sewage = 1 - np.exp(-deso_s * dt_h)
    deso_marine = 1 - np.exp(-deso_m * dt_h)
    ...
    class MPParticle(JITParticle):    
        ...
        # Probabilities #
        abso_probability = Variable('abso_probability', initial = abso_prob)
        deso_s_probability = Variable('deso_s_probability', initial = deso_sewage)
        deso_m_probability = Variable('deso_m_probability', initial = deso_marine)
        # Initialization flag #
